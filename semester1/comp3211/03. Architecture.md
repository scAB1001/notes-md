...
Deciding on software components, their interaction, and their placement leads to an instance of a software architecture, also known as a system architecture [Bass et al., 2003]. There are centralized and decentralized organizations, as well as various hybrid forms.
...
• View all of the HTTP and SSL / HTTPS traffic between your
machine and the Internet, https://www.charlesproxy.com/
#### Decentralized organizations: peer-to-peer systems
Processes are all equal: the functions that need to
be carried out are represented by every process
• Each process will act as a client and a server at the
same time (i.e. acting as a **servant**)

Structured P2P
• Nodes (processes) are organised in an overlay that adheres to a
specific, deterministic topology used to look up data
• Make use of a semantic-free index: each data item is uniquely
associated with a key, in turn used as an index. Common practice: use a
hash function: key(data item) = hash(data item's value)
...
• Looking up data **d** with key **in** {0,1, 2,…24-1} means **routing** request to
node with **identifier** k
• Question: what happens when node with identifier **0111** is requested to
look up the data having key **14**?
...
• Specific class of distributed systems in which client server
solutions are combined with decentralised architectures
• Example: Edge-server systems
• deployed on the Internet where servers are placed at the **edge** of
the network: the boundary between enterprise networks and the
actual Internet, e.g. as provided by **Internet Service providers**
**(ISPs)**
...
• **Edger servers** serve content; a collection of edge servers can
be used to optimise content and application distribution.