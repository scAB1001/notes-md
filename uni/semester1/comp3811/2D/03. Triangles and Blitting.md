...
Problem”: other edges and different types of triangles
...
- Mustn’t draw outside of the framebuffer
Implicit methods:
- Easy: don’t consider points from outside of the framebuffer.
- Explicit methods: :(
...
- Important: just don’t test each sample/pixel individually!

#### Blit
- Variations: blit, bit blit, blt, …
- “Bit block transfer”
...
- “Limited” to format conversions and scaling
##### Essentials
...
- GCC/MSVC weren’t able to do that in a quick test (but moving away from copying single bytes probably has a bigger impact)
...
Threshold:
- Standard alpha: use ≤ 0 as alpha = zero means fully transparent
- Using e.g. ≤ 128 or ≤ 0.5 can result in slightly nicer borders
...
Normally:
- Background: previous contents of image = “destination”
- Foreground: image we’re blitting = “source”
- Alpha blending: linear mix / interpolation between the two
	𝑓𝑖𝑛𝑎𝑙𝐶𝑜𝑙𝑜𝑟 = 𝛼 ⋅ 𝑠𝑜𝑢𝑟𝑐𝑒𝐶𝑜𝑙𝑜𝑟 + (1 − 𝛼) ⋅ 𝑑𝑒𝑠𝑡𝑖𝑛𝑎𝑡𝑖𝑜𝑛𝐶𝑜𝑙𝑜𝑟
	(Important: source and destination colors in **linear RGB! NOT SRGB** )
###### Gotchas
- Linear alpha blending is **order dependent**! (As in layer 1 on top of layer 2 or the other way around)
- This will be a problem in 3D.

Blit – Blending vs Masking
- Alpha = 0.0 has similar effect as masking
- For “software” 2D: masking probably more efficient than blending
- When drawing in 3D, there are subtle differences
- E.g., whether other data (depth) gets updated.
- Masking is easier to use correctly

Blit – Other blending operations
- Alpha blending only one way of blending
- Will look at different blending modes in 3D
- Quick one: additive blending
	𝑓𝑖𝑛𝑎𝑙𝐶𝑜𝑙𝑜𝑟 = 𝑠𝑜𝑢𝑟𝑐𝑒𝐶𝑜𝑙𝑜𝑟 + 𝑑𝑒𝑠𝑡𝑖𝑛𝑎𝑡𝑖𝑜𝑛𝐶𝑜𝑙𝑜𝑟
- Just add colors. Useful in some particle systems (e.g. fire)
- Higher density => brighter color
- Order independent! (Addition is commutative!)

Rasterization - Blending
- Blending is why we need to be careful with rasterization
- Two adjacent triangles that share an edge
- Pixels along the edge must not be drawn twice
- Example: additive blending would add two contributions