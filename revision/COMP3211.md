...
Difficult to scale existing DSs that were designed for LANs because they are based on *synchronous communication*.
- In this form of communication, a party requesting service (client), blocks until a reply is sent back from the server implementing the service e.g., a database transaction. 
	- This works in LANs where communication between two machines is often at worst a few hundred microseconds.
