...
**(EXAMPLE 1)** The Internet Domain Name System (DNS). The DNS name space is hierarchically organized into a tree of domains, which are divided into non-overlapping ZONES. 

These zones allow the DNS to scale through distribution and decentralized administration, allowing different organizations to manage their own portions of the name space independently while maintaining a coherent global system.
...
E.g. `flits.cs.vu.nl` and $R(Z_i) = 128.0.0.1$ be the resolving of zone $i$ to a server address. 
$$\begin{align} R(\text{flits.cs.vu.nl}) = R(Z_1) \\ 
R(\text{flits.cs.vu})= R(Z_2) \\
R(\text{flits.cs})= R(Z_3) \\ 
\end{align}$$
- To resolve this name, 1. pass to server of zone (Z1) which returns the address of the server for Z2, to which the rest of name, `flits.cs.vu`, can be handled. 
	- Z2 will return the address of the server for zone Z3, which is capable of handling the last part of the name and will return the address of the associated host.
