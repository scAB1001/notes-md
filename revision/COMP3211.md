# Content 
## 1. Introduction to Distributed Syste ms Pt.1 - *Chapter 1*
### Definition of a Distributed Syste m (DS)
...
#### Examples of Distributed Syste ms
...
#### Distributed versus Decentralised Syste ms
There are two views of DS:
1. INTEGRATIVE view: connecting existing networked computer syss into a larger a sys.
...
> A DISTRIBUTED syste m is a networked computer sys in which processes and resources are **sufficiently** spread across multiple computers. (expansive view).
...
#### Examples of Decentralised and Distributed Syste ms
1. **Blockchain (distributed ledger)  decentralised syste m**
...
2. **Geographically dispersed  decentralised syste m**
Consider syss that are naturally geographically dispersed. This occurs typically with syss in which an actual location needs to be monitored, for example, in the case of a power plant, a building, a specific natural env, and so on. 
...
### Openness
> An OPEN DS offers components that can easily be used by, or integrated into other syss. An open DS itself will often consist of components that originate from elsewhere.

![[middleware-openness-ex.png\|500]]
They *share* the same *interface* and *communicate* with the same *common protocol* to be able to *interact with services from other open syss*, irrespective of the underlying env.
...
- This sys is *necessarily* placed in *proximity* of the sensor nodes, but is otherwise to be considered to operate independently. 
	- it may operate as a small local DS, storing all recorded events and offering access to remote syss owned by partners in the consortium. 
- Most partners have local DS (often a cluster of computers) that they use to further process the data collected by the telescope. 
	- In this case, the local syss directly access the central point at the telescope using a standard communication protocol. 
...
**(EXAMPLE 3)** DSs spanning multiple admin domains that do not suffer from admin scalability problems like file-sharing P2P networks. 
 ...
 - In these DSs, the end users -not admin entities, collaborate to keep the sys running. 
	 - At best, underlying admin orgs such as Internet Service Providers (ISPs) can police the network traffic that these P2P syss cause, but such efforts are not very effective.
### Scaling Techniques
...
- Asynchronous communication can be used in batch-processing syss and parallel apps where independent tasks can be scheduled for execution while another task is waiting for the communication to complete. 
...
Scalability problems often appear in the form of *performance degradation*; the solution is to REPLICATE components across a DS. It *increases availability* and *balances the load* between components leading to better performance. Also, in geographically widely dispersed syss, having a copy nearby can *hide communication latency* problems.
...
## 2. Types of Distributed Syste ms Pt.2 - *Chapter 1*
There are three types of DS that serve distinct purposes.
### 1. High performance distributed computing syste ms (HPDCS)
#### Distributed Shared Memory syste ms
...
**(CONTEXT)** Distributed Shared Memory syss vs Multiprocessors
...
#### Cluster Computing
Cluster computing syss became popular when the price/performance ratio of personal computers and workstations improved. It made economical sense to build a supercomputer using off-the-shelf technology by simply hooking up a collection of relatively simple computers in a high-speed network. 
- Mostly used for parallel programming in which a single (compute intensive) program is run in parallel on multiple machines.
- Essentially a group of high-end syss connected through a Local Area Network (LAN)
...
These DSs are often constructed as a federation of computer syss, each may fall under a different admin domain: hardware, software and network technology could differ.
...
Most grid-computing software focuses on managing access to different admin domains, and applications to users and virtual orgs. This outlines the architectural issues.
...
- Processors, routers, power and cooling syss- invisible to customers
...
- It is important to realise that these applications are executed in the VENDOR’s cloud.
...
### 2. Distributed information syste ms
#### Context
Many of the existing middleware solutions are the result of working with an infrastructure in which it was easier to integrate applications into an enterprise-wide information sys. 
...
- These so-called **publish/subscribe syss** form an important and expanding class of DS.
- Messages are sent to logical contact point (published), and forwarded to subscribed apps.
### 3. Distributed Syste ms for Pervasive computing (DPS)
...
#### Ubiquitous Computing Syste ms (UCS)
...
_any information that can be used to characterise the situation of entities (i.e., whether a person, place or object) that are considered relevant to the interaction between a user and an application, including the user and the application themselves_. 
...
* **Device Heterogeneity:** Encompasses a wide range of **IP-enabled devices** beyond smartphones/tablets (e.g., IoT sensors, wearables, vehicular syss).
...
**(DATA PROCESSING ARCHITECTURES)** To organise a sensor network as a distributed database, there are essentially two extremes:
...
**(WHAT IS GOOGLE SPANNER?)** Google Spanner is a globally distributed, strongly consistent, relational database service. Its unique achievement is combining the horizontal scalability of NoSQL syss with **Strong ACID transactions** and an SQL interface across data centers worldwide. It was first described in 2012 and is used internally for services like Google Ads, Gmail, and Google Photos before being offered as Cloud Spanner
...
**(CONSISTENCY PROPERTIES)** By default, Spanner provides **External Consistency**, which is the strictest isolation level for transaction-processing syss.
...
This style, promoting clear encapsulation, forms the foundation for **Service-Oriented Architectures (SOA)**, where syss are built by composing independent, often remotely hosted, services (e.g., an online shop using a third-party payment service).
#### Resource-centred
...
The classic **OSI model** has seven layers, but practical syss like **TCP/IP** use fewer.
![[layered-protocols.png\|500]]

**(MIDDLEWARE PROTOCOLS)** This layered model is extended in DSs by **Middleware**, which sits between the OS/transport layers and the application. It provides common services like secure communication, data marshalling, and naming, freeing the application developer from re-implementing them.
...
**Check of Understanding**
> **Question:** What is a major drawback of the transient, synchronous model used in classic client-server syss?
...
**Check of Understanding**
> **Question:** What is the key technological difference between early RPC syss (e.g., SunRPC) and modern ones like gRPC?
...
MOM provides **persistent, asynchronous communication** via messages, leading to loosely coupled, reliable syss.
...
**(ADVANCED MESSAGE QUEUING PROTOCOL - AMQP)** AMQP is an open standard application-layer protocol for MOM, defining how messages are formatted and transmitted between syss (e.g., between a publisher and a queue, or a queue and a consumer).
*   *Example syss:* **RabbitMQ** (a widely-used AMQP broker), **Apache ActiveMQ**. They are used for enterprise integration, task distribution, and in cloud platforms.
...
### Conceptual Design of Software syss
At a conceptual level, software syss are broken into layers, leading to tiered architectures.
...
1.  Integrating full legacy syss (which are themselves 2 or 3-tier) as resources.
2.  Adding dedicated web or API gateway servers as an extra presentation tier.
**Challenge:** N-tier architectures face significant complexity in **integration** due to a lack of universal standards, requiring extensive custom middleware to connect disparate syss.
### Emergence of SOAs
SOA emerged as a direct response to the challenges of N-tier and enterprise application integration. The trend was toward integrating complex, decoupled syss, often from different orgs. SOA facilitates this by treating application **functionality as reusable, loosely-coupled services**.
### Vision
SOA is an architectural style for building applications from **independent services** that communicate via messages. It represents a paradigm shift from tightly-coupled object-oriented syss to **message-oriented**, platform-independent integration. ARCH MODEL:
...
3.  **Adherence to Well-Supported Standards:** SOA relies heavily on global, platform-neutral standards (e.g., XML, SOAP, WSDL) to enable interoperability across orgal boundaries, unlike the often proprietary or bespoke interfaces in N-tier syss.
**Check of Understanding**
...
> *  **Discovered and consumed separately** by the new mobile app, web app, and partner syss.
...
*   **Headers:** Metadata (e.g., `Content-Type: application/json`, `Authorisation: Bearer ...`).
...
**(FUNCTION OF A NAMING SYSTE M)** A naming sys manages **bindings** between names and entity attributes. Its primary function is **name resolution**. Secondary functions include creating/deleting bindings, listing names, and organising the **namespace** (the set of all valid names).
### Classes of naming syste ms
...
### Namespaces
This section focuses on **flat naming syss**, where entities have non-hierarchical, often meaningless identifiers (IDs). The key challenge is **locating an entity when given only its flat ID**.
...
![[arp-pro-ex.png\|500]]
*   **Drawback:** Does **not scale** beyond local networks; generates excessive traffic in large syss.
...
**(DOMAIN NAME SYSTE M - DNS)** DNS is the canonical distributed, hierarchical naming sys for the internet.
...
A fundamental issue in DSs is **replication**: maintaining multiple copies of the same data (e.g., a file, database record) across different machines.
...
*Solution:* To build scalable syss, we often **relax the consistency requirements**, accepting that replicas may be temporarily inconsistent. The specific rules for what is acceptable are defined by **consistency models**.
...
**(CONSISTENCY UNIT - CONIT)** The data unit over which these deviations are measured is called a **Conit**. An application defines a Conit, and the sys ensures deviations stay within specified bounds.
...
*   An application could specify: "Never let the numerical deviation for the 'fuel cost' Conit exceed £1,000." The sys would then trigger synchronisation before this bound is breached.
...
Being fault tolerant is strongly related to what are called dependable syss.
...
![[crash-ex-1.png\|500]]This approach works as long as processes do not fail. Problems start when a process $P_i$ detects, during round $r$, that, say process $P_k$ has crashed. 
...
![[byzantine=generals-problem.png\|500]]
...
![[arb-consensus-3.png.png.png\|500]]
