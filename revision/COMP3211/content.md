...
| REST PO example                      | Web Service PO                           |
| ------------------------------------ | ---------------------------------------- |
| ![[rest-purchase-order-ex.png\|300]] | ![[web-service-pruchase-order.png\|300]] |
#### Claimed benefits
...
| HTTP Anatomy                      | GET/POST Response             |
| --------------------------------- | ----------------------------- |
| ![[anatomy-of-http-req.png\|250]] | ![[get-post-req-ex.png\|400]] |
**(ANATOMY OF HTTP RESPONSE)** The server replies with:

| Anatomy of HTTP Response           |                                                                                                                                                                                           |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![[anatomy-of-http-resp.png\|300]] | **Status Line:** `HTTP/<version> <Status Code> <Reason>` (e.g., `HTTP/1.1 200 OK`).<br>**Headers:** Metadata about the response.<br>**Body (optional):** The req repr (e.g., HTML, JSON). |
...
**(REST APIs: Examples)** Major platforms provide RESTful APIs for programmatic access:
...
| --------------------------- | -------------------------------- |
| ![[aws-lambda-ex.png\|360]] | ![[aws-lambda-arch-ex.png\|330]] |
...
| Naming Filesystem Ex             | FS Namespace, common root node Ex.  |
| -------------------------------- | ----------------------------------- |
| ![[naming-file-sys-ex.png\|200]] | ![[single-root-naming-ex.png\|400]] |
#### Name resolution
...
## 13. Consistency and Replication Pt.2 - *Chapter 7*
...
![[primary-bakcup-remote-write-prot.png\|300]]
...
![[primary-bakcup-local-write-prot.png\|300]]
...
This approach works as long as processes do not fail. Problems start when a process $P_i$ detects, during round $r$, that, say process $P_k$ has crashed. 
...
**(EXAMPLE)** assume we have a process group of four processes {P1, . . . , P4} and that $P_1$ crashes during round $r$. Also, assume that $P_2$ receives the list of proposed commands from $P_1$ before it crashes, but that $P_3$ and $P_4$ do not (in other words, $P_1$ crashes before it got a chance to send its list to $P_3$ and $P_4$).
![[crash-ex-1.png\|400]]
...
**(EXAMPLE)** consider the situation of tolerating the failure of a single process, that is, $k = 1$. *Solution*: $3k+1$
![[arb-consensus-2.png.png\|300]]
...
#### System Models
Failures are complex, both nodes and networks may be faulty! Sys designers make explicit **assumptions (models)** about:
...
| Cloud Concept                      | Layered Cloud Arch               |
| ---------------------------------- | -------------------------------- |
| ![[concept-cloud-vision.png\|300]] | ![[layered-cloud-arch.png\|300]] |
1. *The cloud provides a simplified, service-oriented interface to complex, pooled resources.*
2. *The stack shows how virtualisation and management software abstract the physical infrastructure.*
