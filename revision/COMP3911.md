...
The DNS Server has no way of verifying this information so it acceptsit and caches the data.

## Spoofing Attacks
A situation where a person or program successfully identifies as another by falsifying data to gain an advantage. 

Its the process of fooling DNS/ARP software into giving an **attacker-selected address** as the 
- IP address for a given domain (DNS)
- MAC address for a given IP address (ARP)

E.g. If I attempt to navigate to www.google.com, the DNS might resolve to an IP address controlled by the attacker.
E.g. It would give the MAC address of a machine controlled by the attacker.