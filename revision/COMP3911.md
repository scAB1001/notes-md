...
No proces can read data at a higher level (*no read up*)

Subject *s* can read object *o*, $$iff \ \ L(o) \le L(s)$$ and *s* has permission to write *o* .
This also means *no write down*. Thanks to this mandatory access contorl, information can only flow upwards. Protects against trojans.
### Formal BLP
...
Given two labels get the LUB and GLB. E.g. *(Secret, {Crypto, Foregin})* and *(Top Secret, {Crypto})*
GLB from meet() is *(Secret, {Crypto})* because that's where their two edges meet at the bottom.
LUB from join() is *(Top Secret, {Crypto, Foregin})* becaus
