...
#### DNS/ARP Spoofing
Fooling DNS/ARP software into givinG an **attacker-selected address** as the 
- IP address for a given domain (DNS)
- MAC address for a given IP address (ARP)

E.g. If I attempt to navigate to www.google.com, the DNS might resolve to an IP address controlled by the attacker instead of Google.
