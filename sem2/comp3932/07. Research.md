# Feasibility and Usefulness of a Joystick-Guided Robotic Scope Holder (Soloassist) in Laparoscopic Surgery
https://karger.com/vis/article/34/1/37/328326/Feasibility-and-Usefulness-of-a-Joystick-Guided
### Solo Assist II
https://infectioncontrol.invitro.com.au/brands/aktormed/solo-assist-ii/

#### Paediatric Minimally Invasive Surgery
The **SOLO ASSIST II** with Voice Control is an Intelligent Robotic Assistant System for .
https://infectioncontrol.invitro.com.au/brands/aktormed/paediatric-minimally-invasive-surgery

_"With the goal of reducing the morbidity associated with open surgery, minimally invasive surgery in children is increasingly being performed as laparoscopic and robotic patients appear to be experiencing shorter hospital stays, decreased pain medication requirements, and the potential for improved cosmesis.” -_ Lee D.J, Kim P.H, Koh C.J. Current Trends in Pediatric Minimally Invasive Urologic Surger
### Voice control for SOLOASSIST
https://infectioncontrol.invitro.com.au/brands/aktormed/voice-control-for-soloassist
#### Key features
- Speaker independent - no speech training needed
- high responsive - average speech-to-movement time less than 200 ms
- precise – more than 95% recognition rate
- compatible with joystick control
#### Intuitive set of commands
The command set of the SOLOASSIST Voice Control SIIVoice allows a precise control over the positioning of the endoscope during operation. Any command sequence is started with the hotword SOLO. After recognition of the hotword, commands are accepted for five seconds. If no command is recognized within this time SIIVoice returns to standby mode, waiting for the hotword again.
#### Moving the endoscope
The four main directions (UP, DOWN, LEFT, RIGHT) as well as zooming (IN, OUT) can be combined with a distance modifier (ONE, TWO, THREE, FIVE, TEN) to enable an accurate control over the scope. In addition it is possible to use the keywords UPPER and LOWER to move diagonal. The command MOVE repeats the last movement.
#### Saving positions
Sometime it may be helpful to memorize a special endoscope position. With the SAVE and RESTORE keywords you can save up to three positions and restore this view when needed. With RETURN it is possible to undo the last movement.
#### Speed settings
With the SPEED keyword followed by ONE, TWO or THREE the speed of the movements of the SOLOASSIST can be adjusted to three preset speeds.
#### Are you at risk of suffering from Surgical Strain?
**RESEARCH SHOWS**: 1 IN 5 SURGEONS ARE AFFECTED and forced into early retirement, because of Surgical Strain. _The Journal of the American Medical Association - December 2017_

- Cervical Spine Disease 17%
- Rotator Cuff Pathology 18%
- Degenerative Lumbar Spine 19%
- Carpal Tunnel Syndrome 9%

https://infectioncontrol.invitro.com.au/brands/aktormed/are-you-at-risk-of-suffering-from-surgical-strain
#### Prevalence of Work-Related Musculoskeletal Disorders Among Surgeons
https://www.researchgate.net/publication/322087192_Prevalence_of_Work-Related_Musculoskeletal_Disorders_Among_Surgeons_and_Interventionalists_A_Systematic_Review_and_Meta-analysis

_"Importance Physicians in procedural specialties are at high risk for work-related musculoskeletal disorders (MSDs). This has been called “an impending epidemic” in the context of the looming workforce shortage; however, prevalence estimates vary by study. Objectives To estimate the prevalence of work-related MSDs among at-risk physicians and to evaluate the scope of preventive efforts..."_ - Sherise Epstein, MPH; Emily H. Sparer, ScD; Bao N. Tran, MD; Qing Z. Ruan, MD; Jack T. Dennerlein, PhD; Dhruv Singhal, MD; Bernard T. Lee, MD, MPH, MBA

# Laparoscopic inguinal hernia repair with a joystick-guided robotic scope holder (Soloassist II®): retrospective comparative study with human assistant
# Surgeons operate remotely using PlayStation game controller
https://surgery.international/surgeons-operate-remotely-using-playstation-game-controller/
# The Benefits of Joysticks for Ultrasound Machines: Enhanced Imaging and Diagnosis
https://parts.ruffycontrols.com/the-benefits-of-joysticks-for-ultrasound-machines-enhanced-imaging-and-diagnosis/
# A Gesture-based Tool for Sterile Browsing of Radiology Images
https://pmc.ncbi.nlm.nih.gov/articles/PMC2410001/
# ‘Gamer’ technology helps Sarah Cannon detect lung cancer at earlier stages
https://hcahealthcaretoday.com/2020/11/30/new-technology-at-sarah-cannon-to-help-diagnose-lung-cancer-earlier/
# Immersive 3D virtual reality imaging in planning minimally invasive and complex adult cardiac surgery
# A VR-based volumetric medical image segmentation and visualization system with natural human interaction
# A virtual surgical prototype system based on gesture recognition for virtual surgical training in maxillofacial surgery
# Mixed reality for preoperative planning and intraoperative assistance of surgical correction of complex congenital heart defects
# Virtual reality three-dimensional echocardiographic imaging for planning surgical atrioventricular valve repair
# Surgeon perspectives on a virtual reality platform for preoperative planning in complex bone sarcomas
# Analyzing the effectiveness and satisfaction of hand tracking vs. controllers among VR-experienced users
# Controllers or Bare Hands? A Controlled Evaluation of Input Techniques on Interaction Performance and Exertion in Virtual Reality
# Comparing hand-based and controller-based interactions in virtual reality learning: effects on presence and interaction performance
# Virtual reality locomotion methods differentially affect spatial orientation and cybersickness during maze navigation
# Unlocking the Potential of Immersive VR Systems: The Role of Gesture and Natural Controls
# Studying the Mental Effort in Virtual Versus Real Environments

### 1. The Global/Domain Context
**The Goal:** Prove that your project solves a real-world medical problem (The "So What?" test).
**What to search for:** `"Virtual reality" AND "preoperative surgical planning" AND "spatial understanding"` or `"3D anatomy visualization" AND "surgical outcomes"`.

**Core Insights to look for (and cite):**
* **The Mental 3D Reconstruction Problem:** Traditionally, surgeons look at 2D slices (CT/MRI scans) and have to build a 3D model of the patient’s anatomy *in their minds*. Literature frequently cites this as a high-cognitive-load task that takes years to master.
* **Altered Surgical Plans:** Recent systematic reviews (like those published in *SPIE Journal of Medical Imaging* or *Surgical Endoscopy*) show that when surgeons use VR to view patient-specific anatomy, they alter their surgical plans in **40% to 52% of cases** because they can spot anatomical variants and spatial relationships they missed on 2D monitors.
* **Clinical Outcomes:** Look for papers demonstrating that VR planning leads to reduced operative times, reduced intraoperative blood loss, and fewer fluoroscopic (radiation) images needed during surgery. 

### 2. The HCI Bottleneck (The Problem)
**The Goal:** Prove that while VR is great, the *current hardware* (joysticks) limits its potential for non-technical users.
**What to search for:** `"Virtual reality" AND "controllers vs hand tracking"` or `"indirect hardware interfaces" AND "cognitive load" AND "VR"`.

**Core Insights to look for (and cite):**
* **Mapping Dissonance:** Controllers create a disconnect between human intent and digital action. You must explain that pushing a plastic thumbstick to rotate a virtual liver is an "abstract control scheme." It forces the brain to translate a physical gesture into a digital output, which increases cognitive friction.
* **The Barrier to Entry:** Medical professionals are not inherently gamers. Studies frequently note that holding a controller with triggers, grip buttons, and thumbsticks is intimidating and requires an onboarding process that disrupts the clinical workflow.
* **The Baseline Defense:** You must also find papers that defend controllers (e.g., studies showing controllers provide "binary certainty" with button presses and exact precision). You need this to justify why the Varjo controller is a valid baseline for your comparative user study.
### 3. Proposed Solutions (Hand-Tracking & NUI)
**The Goal:** Position optical hand-tracking (Natural User Interfaces) as the logical evolution for medical XR.
**What to search for:** `"Optical hand tracking" AND "Ultraleap" OR "gesture recognition"` or `"Natural User Interfaces" AND "embodied cognition VR"`.

**Core Insights to look for (and cite):**
* **Zero-Friction Computing:** Hand tracking democratizes XR. Because everyone knows how to use their hands, the act of reaching out to grab a virtual object lowers the cognitive load and provides a direct 1:1 kinematic mapping.
* **Sense of Embodiment:** Research shows that bare-hand interaction significantly enhances the user's sense of body ownership and presence in the virtual environment compared to holding a proxy object (the controller).
#### Argument 1: Natural User Interfaces (NUI) Reduce Cognitive Load
You must prove mathematically that using hands is easier on the brain than using a controller. You are looking for comparative studies that use the same metrics you are using (NASA-TLX and Task Completion Time).

* **Exact Search Strings:** * (Natural User Interface OR NUI OR bare-hand tracking) AND (cognitive load OR NASA-TLX) AND virtual reality
    * hand tracking AND spatial manipulation AND 6-DoF
* **What to extract for your report:** Look for papers that show a statistically significant reduction in the **Mental Demand** sub-scale of the NASA-TLX when users switch from controllers to hand-tracking. You want to quote their findings on how direct hand manipulation eliminates the need for the brain to translate a physical thumb movement into a digital rotation.
#### Argument 2: Embodied Cognition and Proprioception
This is how you score top marks. Embodied cognition is the psychological theory that our physical bodies shape our thinking. In a medical context, a surgeon's spatial understanding is tied to their physical hands.

* **Exact Search Strings:**
    * embodied interaction AND virtual reality AND (medical OR anatomy OR surgery)
    * proprioception AND virtual reality AND grasping
* **What to extract for your report:** Find papers discussing how doctors rely on proprioception (the body's ability to sense movement and location). Extract quotes arguing that when a user reaches out and pinches a virtual liver, they are leveraging years of real-world evolutionary motor skills, whereas holding a joystick forces them to learn an entirely new, artificial motor skill.
#### Argument 3: The Validation of Optical Tracking Technology
You need to justify why optical tracking (specifically infrared camera-based systems like the Ultraleap Controller 2) is now precise enough for clinical use, distinguishing it from older, inaccurate technologies (like the original Xbox Kinect).

* **Exact Search Strings:**
    * (Leap Motion OR Ultraleap) AND precision AND virtual reality
    * optical hand tracking AND evaluation AND dexterity
* **What to extract for your report:** Look for quantitative evaluations of tracking latency and millimeter accuracy. You want to cite papers that prove modern optical tracking operates at sub-millimeter precision with low enough latency to prevent simulator sickness. This proves the hardware is enterprise-grade and capable of handling complex geometry like a 3D liver mesh.
### 4. Critical Nuances (Showing "Deep Understanding")
**The Goal:** Examiners award marks $\ge 80\%$ to students who critically evaluate their own solutions. You must research the *flaws* of your proposed system.
**What to search for:** `"Hand tracking" AND "Gorilla Arm fatigue"` or `"optical occlusion" AND "VR tracking latency"`.

**Core Insights to look for (and cite):**
* **"Gorilla Arm" Fatigue:** A well-documented physiological phenomenon in HCI where users experience severe shoulder and arm fatigue from holding their arms extended in mid-air without physical resting support.
* **The Missing Haptics Dilemma:** Interacting with a virtual liver using bare hands feels like "pushing air." Without the rumble or resistance of a controller, the brain lacks tactile confirmation that a "grab" was successful.
* **Optical Occlusion:** The Ultraleap cameras cannot see through solid objects. If one hand crosses over the other (e.g., during your bimanual scaling task), the tracking fails.

**A tip for your literature review:** Don't just list the papers. Synthesize them. For example: *"While [Author A] proved that VR reduces surgical planning time, [Author B] demonstrated that traditional controllers introduce a steep learning curve. This project bridges that gap by..."*


Here is your synthesized literature review foundation. I have categorized your 60 journal articles into the four critical themes we established for Chapter 1. 

Within each category, the papers are ordered by **usefulness** (a combination of recency, direct relevance to 3D manipulation, and methodological similarity to your project). Papers at the top of each list are your "Gold Standard" citations; papers at the bottom are either too old, too tangential (e.g., VR locomotion/walking instead of hand-manipulation), or redundant.

***

### 1. The Global/Domain Context
**Goal:** Prove that 3D VR surgical planning improves outcomes and justifies your project's existence.
*(You have an incredibly strong dataset here. Use the top 5 to prove VR is the standard of care).*

**High Priority (Directly relevant to surgical planning/outcomes):**
1. Translating high-precision mixed reality navigation from lab to operating room: design and clinical evaluation, 2025
2. Mixed reality for preoperative planning and intraoperative assistance of surgical correction of complex congenital heart defects, 2025
3. Effect of 3-Dimensional Virtual Reality Models for Surgical Planning of Robotic-Assisted Partial Nephrectomy on Surgical Outcomes: A Randomized Clinical Trial, 2019 *(Crucial: It’s a Randomized Clinical Trial proving outcomes).*
4. Immersive 3D virtual reality imaging in planning minimally invasive and complex adult cardiac surgery, 2020
5. A review of the evidence for training effectiveness with virtual reality technology, 2021
6. Immersive virtual reality for surgical training: a systematic review, 2021
7. Clinical Applications of Mixed Reality and 3D Printing in Congenital Heart Disease, 2022
8. Virtual reality three-dimensional echocardiographic imaging for planning surgical atrioventricular valve repair, 2021
9. Immersive Virtual Reality Heart Models for Planning of Transcatheter Paravalvular Leak Closure: A Feasibility Study, 2021
10. Immersive virtual reality surgical planning of minimally invasive coronary artery bypass for kawasaki disease, 2020

**Medium Priority (Contextual/Broader Medical XR):**
11. XR (extended reality: Virtual reality, augmented reality, mixed reality) technology applications in orthopedic field, 2025
12. Augmented reality guidance improves accuracy of orthopedic drilling procedures, 2024
13. Virtual anatomical and endoscopic exploration method of internal human body for training simulator, 2020
14. What is your reality? virtual, augmented, and mixed reality in plastic surgery training, education, and practice, 2020
15. Virtual, augmented, and mixed reality applications in orthopedic surgery, 2020
16. The application of virtual reality and augmented reality in oral and maxillofacial surgery, 2019
17. Virtual reality for preoperative planning in large ventricular septal defects, 2019
18. Effectiveness of immersive virtual reality in surgical training-a randomized control trial, 2018
19. Operating Room Performance Improves after Proficiency-Based Virtual Reality Cataract Surgery Training, 2017
20. Effectiveness of virtual reality training in orthopaedic surgery, 2016

**Low Priority (Outdated):**
21. Virtual reality training improves operating room performance results of a randomized, double-blinded study, 2002
22. VR interaction techniques for medical imaging applications, 1998

---

### 2. The HCI Bottleneck (The Problem)
**Goal:** Prove that current interfaces (controllers/joysticks) cause cognitive load and mapping dissonance. 

**High Priority:**
1. A comparison of input devices for precise interaction tasks in VR-based surgical planning and training, 2022 *(This is your bridge paper between Context and HCI).*
2. Three-dimensional (3D) stimuli are always better than two-dimensional (2D) multi-tasking? A high cognitive load in 3D-MATB-II, 2025
3. Cognitive load/flow and performance in virtual reality simulation training of laparoscopic surgery, 2021
4. Studying the mental effort in virtual versus real environments, 2019

**Medium/Low Priority (Tangential - Joystick use in Laparoscopy/Locomotion, not VR 6-DoF):**
5. Laparoscopic inguinal hernia repair with a joystick-guided robotic scope holder (Soloassist II®): retrospective comparative study with human assistant, 2019
6. Feasibility and Usefulness of a Joystick-Guided Robotic Scope Holder (Soloassist) in Laparoscopic Surgery, 2018
7. The role of hand motion connectivity in the performance of laparoscopic procedures on a virtual reality simulator, 2013
8. Comparison of laparoscopic colectomy with and without the aid of a robotic camera holder, 2002
9. Locomotion in place in virtual reality: A comparative evaluation of joystick, teleport, and leaning, 2021
10. Evaluation of locomotion techniques for room-scale VR: Joystick, teleportation, and redirected walking, 2018
11. Virtual reality locomotion methods differentially affect spatial orientation and cybersickness during maze navigation, 2025

---

### 3. Proposed Solutions (Hand-Tracking & NUI)
**Goal:** Prove that optical tracking reduces friction and enables embodied interaction.
*(You have a goldmine of recent 2023-2026 head-to-head comparisons here).*

**High Priority (Direct Controller vs. Hand Comparisons):**
1. Analyzing the effectiveness and satisfaction of hand tracking vs. controllers among VR-experienced users, 2026
2. Comparing hand-based and controller-based interactions in virtual reality learning: effects on presence and interaction performance, 2025
3. Controllers or Bare Hands? A Controlled Evaluation of Input Techniques on Interaction Performance and Exertion in Virtual Reality, 2023
4. Comparison of a vr stylus with a controller, hand tracking, and a mouse for object manipulation and medical marking tasks in virtual reality, 2023
5. Unlocking the Potential of Immersive VR Systems: The Role of Gesture and Natural Controls, 2023
6. Is embodied interaction beneficial? A study on navigating network visualizations, 2023 *(Use this for the "Embodied Cognition" argument).*
7. Immersive virtual reality for upper limb rehabilitation: comparing hand and controller interaction, 2023

**Medium Priority (Gesture Tech in Surgery):**
8. A virtual surgical prototype system based on gesture recognition for virtual surgical training in maxillofacial surgery, 2023 (and its 2022 predecessor).
9. A Comparative Study of the User Experience of Controller and Hand-VirTracking Interactions in a tual Environment, 2022
10. Hand tracking for immersive virtual reality: Opportunities and challenges, 2021
11. Hand gesture recognition based on computer vision: a review of techniques, 2020
12. Touchless interaction with software in interventional radiology and surgery: a systematic literature review, 2017
13. A gesture-controlled projection display for CT-guided interventions, 2016
14. Gesture recognition by instantaneous surface EMG images, 2016
15. Touchless gesture user interface for interactive image visualization in urological surgery, 2012

---
### 4. Critical Nuances (Showing "Deep Understanding")
**Goal:** Evaluate the *flaws* of your own system (fatigue, lack of haptics, precision errors).

**High Priority:**
1. Absence of Inertial Load on Hand Decreases Task Performance in Virtual Reality Interaction, 2024 *(Massive paper for your report: Proves that holding a "weightless" virtual liver causes proprioceptive issues).*
2. Force Feedback-Based Adaptive Sensor Fusion Algorithm for Precision Enhancement in VR-Based Medical Training Systems, 2026
3. Evaluation of the impact of different levels of self-representation and body tracking on the sense of presence and embodiment in immersive VR, 2021
4. Spatial orientation in virtual environment compared to real-world, 2021
5. Virtual reality sickness: A review of causes and measurements, 2020

**Medium/Low Priority (Tangential/Old Haptics):**
6. Evaluation of locomotion methods in virtual reality navigation environments: an involuntary position shift and task performance, 2021
7. The role of haptics in medical training simulators: A survey of the state of the art, 2011
8. Haptic feedback in robot-assisted minimally invasive surgery, 2009

***
### Gap Analysis: What You Are Missing
Your list is heavily skewed toward the medical context (Cat 1) and proposed solutions (Cat 3), which is great. However, **Category 2 (The HCI Bottleneck)** and **Category 4 (Critical Nuances)** have specific, glaring gaps that an examiner will look for. 
#### Gap 1: The Joystick "Mapping Dissonance" (Cat 2)
Most of your joystick papers are about *laparoscopic robotic cameras*, not manipulating 3D VR objects. You need a paper that explicitly states that standard VR controllers force the user to translate spatial intent into abstract button presses.
* **What to search for:** `"virtual reality" AND "controller" AND "mapping dissonance"` OR `"6-DoF" AND "spatial manipulation" AND "cognitive load"`
* **Look for:** Papers from the ACM CHI Conference discussing spatial mapping friction.
#### Gap 2: Gorilla Arm Fatigue (Cat 4)
You have an excellent paper on the *absence of inertial load*, but you are missing the canonical HCI critique of vertical touchless interfaces: ergonomic shoulder fatigue.
* **What to search for:** `"Gorilla arm effect" AND "virtual reality"` OR `"hand tracking" AND "ergonomic fatigue" AND "mid-air interaction"`
* **Look for:** Studies that measure physical exertion (like EMG sensor readings of the deltoid muscle) during prolonged mid-air hand tracking.
#### Gap 3: Optical Occlusion (Cat 4)
You need one technical paper explaining *why* hand tracking sometimes fails (e.g., when one hand crosses the other, the camera loses line-of-sight).
* **What to search for:** `"Ultraleap" OR "Leap motion" AND "optical occlusion" AND "tracking accuracy"`


To be completely direct: this specific Table of Contents from the ACM CHI proceedings is heavily skewed toward Augmented Reality (AR) and general workspace interfaces, rather than the highly specific medical VR and hardware evaluation you are conducting. 

Because of this, it does not perfectly fill the specific "Gap 1: Mapping Dissonance" or "Gap 2: Gorilla Arm" holes we identified earlier. However, there are **six papers** here that are highly relevant to your "Critical Nuances" section, your methodology, and the technical background of your project. 
### 1. To Fill the "Critical Nuances" Gaps (Haptics & Mid-Air Interaction)
While these don't address "Gorilla arm" or "Optical Occlusion" directly, they perfectly address the difficulty of interacting with virtual space using bare hands (the lack of physical feedback).

* **Landing Windows Method as Soft Visual Constraints for Mid-Air Interactions**
  * **Why it’s useful:** When a user tries to grab your virtual liver using hand-tracking, their hand will pass straight through the mesh because there is no physical resistance. This paper investigates "soft visual constraints"—using visual UI cues to compensate for the lack of haptic feedback during mid-air interaction. You can cite this to show you understand the limitations of bare-hand tracking.
* **Probabilistic In-Plane Detection for Mid-Air Virtual Surface Interactions**
  * **Why it’s useful:** This will likely discuss the mathematical and technical inaccuracies of free-hand tracking in mid-air. It is an excellent source to cite when discussing why optical tracking algorithms sometimes struggle to determine exact spatial intent without physical buttons.
* **VR Lumbar Puncture Simulation Tactile Device (Poster Abstract)**
  * **Why it’s useful:** This is a direct medical VR simulation paper focused on haptics. You can contrast this approach (building a physical tactile device for a specific medical procedure) with your approach (using bare-hand optical tracking for generalized manipulation), discussing the trade-offs of each.
### 2. Relevant for Your Methodology (Workload Evaluation)
* **Foot Placement Feedback in Physical Training: Effects of Spatial User Interfaces on Performance and Workload in Virtual Reality**
  * **Why it’s useful:** This paper evaluates "Spatial User Interfaces" against "Workload" (almost certainly using the NASA-TLX or similar cognitive load metrics). Even though it is about feet instead of hands, it provides a high-tier CHI reference to justify your specific evaluation methodology (measuring the cognitive workload of a spatial task).
### 3. Relevant for Technical Architecture (Unity & Vision)
* **SurrealityCV: An Open-Source, Cross-Platform Unity Library for Mixed Reality Computer Vision (Poster Abstract)**
  * **Why it’s useful:** This discusses integrating computer vision into Unity for Mixed Reality. It is a highly relevant citation for your Section 1.4.3 (Natural User Interfaces and Computer Vision), demonstrating the current state-of-the-art for Unity-based camera tracking pipelines.
* **Depth-Aware Immersive Visualization of Boundaries Using Particles in VR**
  * **Why it’s useful:** This relates directly to the user's spatial awareness and depth perception in a headset. You can tie this into your Section 1.4.5 (High-Fidelity Rendering), arguing that precise depth-awareness is critical for surgical planning and requires high-fidelity rendering pipelines like Unity HDRP.

***
### What You Still Need to Search For (The Unfilled Gaps)
Since this specific conference list didn't have the exact terminology needed for Gaps 1 and 2, you must still do a targeted search on Google Scholar or IEEE Xplore for these two specific concepts:

1. **The Joystick Mapping Problem:** Search exactly for `"virtual reality" AND "mapping dissonance" AND "controllers"`. You need a paper that explicitly states that thumbsticks force the brain to perform a cognitive translation step.
2. **Gorilla Arm Fatigue:** Search exactly for `"Gorilla arm" AND "mid air interaction" AND "fatigue"`. You need one paper that proves holding hands out in front of the body without support causes rapid physiological strain.

---
### 1. The Global/Domain Context
**Goal:** Prove that 3D VR surgical planning improves outcomes and justifies your project's existence.

**🥇 Gold Standard (High Priority - Cite these to prove clinical necessity):**
1. Translating high-precision mixed reality navigation from lab to operating room: design and clinical evaluation, 2025
2. Mixed reality for preoperative planning and intraoperative assistance of surgical correction of complex congenital heart defects, 2025
3. Effect of 3-Dimensional Virtual Reality Models for Surgical Planning of Robotic-Assisted Partial Nephrectomy on Surgical Outcomes: A Randomized Clinical Trial, 2019
4. Immersive 3D virtual reality imaging in planning minimally invasive and complex adult cardiac surgery, 2020
5. A review of the evidence for training effectiveness with virtual reality technology, 2021
6. Immersive virtual reality for surgical training: a systematic review, 2021
7. Clinical Applications of Mixed Reality and 3D Printing in Congenital Heart Disease, 2022

**🥈 Supporting Literature (Medium/Low Priority - Contextual Medical XR):**
8. XR (extended reality: Virtual reality, augmented reality, mixed reality) technology applications in orthopedic field, 2025
9. Augmented reality guidance improves accuracy of orthopedic drilling procedures, 2024
10. Virtual reality three-dimensional echocardiographic imaging for planning surgical atrioventricular valve repair, 2021
11. Immersive Virtual Reality Heart Models for Planning of Transcatheter Paravalvular Leak Closure: A Feasibility Study, 2021
12. Immersive virtual reality surgical planning of minimally invasive coronary artery bypass for kawasaki disease, 2020

_(Remaining 13 older/tangential medical XR papers: Plastic surgery training 2020, Orthopedic surgery 2020, Ventricular septal defects 2019, Maxillofacial surgery 2019, Cataract surgery 2017, Orthopaedic surgery 2016, Medical imaging 1998, Laparoscopic colectomy 2002, etc.)_

---
### 2. The HCI Bottleneck (The Problem)
**Goal:** Prove that controllers cause cognitive load and "mapping dissonance" during spatial tasks.

**🥇 Gold Standard (High Priority - The friction of controllers):**
1. XR Cricothyroidotomy and Intraosseous Access: A Preliminary Study on Usability and Presence between VR/MR and Hand Tracking/Controller Interactions, 2025 _(Crucial: Direct medical comparison)._
2. A comparison of input devices for precise interaction tasks in VR-based surgical planning and training, 2022
3. Three-dimensional (3D) stimuli are always better than two-dimensional (2D) multi-tasking? A high cognitive load in 3D-MATB-II, 2025
4. Cognitive load/flow and performance in virtual reality simulation training of laparoscopic surgery, 2021
5. Studying the mental effort in virtual versus real environments, 2019
6. Understanding the attention demand of touch and tangible interaction on a composite task, 2019

**🥈 Supporting Literature (Medium/Low Priority - Tangential Locomotion/Joystick studies):**
7. Locomotion in place in virtual reality: A comparative evaluation of joystick, teleport, and leaning, 2021
8. Evaluation of locomotion methods in virtual reality navigation environments... 2021
9. Virtual reality locomotion methods differentially affect spatial orientation and cybersickness... 2025

_(Remaining papers regarding joystick-guided laparoscopic camera holders from 2019, 2018, 2013)._

---
### 3. Proposed Solutions (Hand-Tracking & NUI)
**Goal:** Position optical hand-tracking and the Leap Motion 2 as the logical evolution for medical XR.

**🥇 Gold Standard (High Priority - Hand vs. Controller & Hardware Validation):**
1. Analyzing the effectiveness and satisfaction of hand tracking vs. controllers among VR-experienced users, 2026
2. Comparative Analysis of the Accuracy and Robustness of the Leap Motion Controller 2, 2025 _(Crucial: Evaluates your exact hardware)._
3. Comparing hand-based and controller-based interactions in virtual reality learning: effects on presence and interaction performance, 2025
4. Controllers or Bare Hands? A Controlled Evaluation of Input Techniques on Interaction Performance and Exertion in Virtual Reality, 2023
5. Comparison of a vr stylus with a controller, hand tracking, and a mouse for object manipulation and medical marking tasks in virtual reality, 2023
6. Unlocking the Potential of Immersive VR Systems: The Role of Gesture and Natural Controls, 2023
7. Is embodied interaction beneficial? A study on navigating network visualizations, 2023

**🥈 Supporting Literature (Medium Priority - General Gesture Tech/Older Hardware):**
8. Evaluation of XR device's real-world tracking accuracy and depth perception... 2025
9. A Novel Sensor Fusion Approach for Precise Hand Tracking in Virtual Reality-Based Human—Computer Interaction, 2023
10. Robust Optical Based Hand Interaction for Virtual Reality, 2022
11. Hand tracking for immersive virtual reality: Opportunities and challenges, 2021
12. Object manipulation and deformation using hand gestures, 2021

_(Remaining 18 papers on older Leap Motion validations from 2013-2020, Kinect sensors, EMG gestures, and generic computer vision gesture reviews)._

---
### 4. Critical Nuances (Showing "Deep Understanding")
**Goal:** Evaluate the _flaws_ of your system (Gorilla Arm fatigue, optical occlusion, lack of haptics).

**🥇 Gold Standard (High Priority - The flaws of mid-air interaction):**
1. Quantifying the 'Gorilla Arm' Effect in a Virtual Reality Text Entry Task via Ray-Casting: A Preliminary Single-Subject Study, 2023 _(Use to define the phenomenon)._
2. Absence of Inertial Load on Hand Decreases Task Performance in Virtual Reality Interaction, 2024 _(Use to explain lack of haptic feedback on the virtual liver)._
3. The effect of hands synchronicity on users perceived arms Fatigue in Virtual reality environment, 2023 _(Use to justify your bimanual scaling task design)._
4. Modeling cumulative arm fatigue in mid-air interaction based on perceived exertion and kinetics of arm motion, 2017
5. Real-Time Hand Tracking under Occlusion from an Egocentric RGB-D Sensor, 2017 _(Defines the optical tracking blindspot)._
6. Landing Windows Method as Soft Visual Constraints for Mid-Air Interactions, 2025 _(A solution to lack of haptics)._
7. Probabilistic In-Plane Detection for Mid-Air Virtual Surface Interactions, 2025

**🥈 Supporting Literature (Medium/Low Priority - Peripheral Fatigue/Haptic Studies):**
8. Dual Leap Motion Controller 2: A Robust Dataset for Multi-view Hand Pose Recognition, 2024 _(Use for Future Work/Self-Appraisal)._
9. Force Feedback-Based Adaptive Sensor Fusion Algorithm for Precision Enhancement in VR-Based Medical Training Systems, 2026
10. Effects of different fatigue locations on upper body kinematics and inter-joint coordination in a repetitive pointing task, 2019
11. Outline Pursuits: Gaze-assisted Selection of Occluded Objects in Virtual Reality, 2020

_(Remaining papers on generalized VR sickness, 2009 haptics, and basic pointing task variability)._

---
### 5. Methodology & Architecture (For Chapter 2 & 3)
**Goal:** Prove your evaluation metrics and software pipeline use validated scientific/industry standards.

**🥇 Gold Standard (Use these explicitly when designing your experiment):**
1. Development of NASA-TLX (Task Load Index): Results of Empirical and Theoretical Research, 1988 _(Mandatory citation for using the NASA-TLX)._
2. Foot Placement Feedback in Physical Training: Effects of Spatial User Interfaces on Performance and Workload in Virtual Reality, 2025 _(Modern CHI validation of measuring VR workload)._
3. SurrealityCV: An Open-Source, Cross-Platform Unity Library for Mixed Reality Computer Vision, 2025 _(Use to justify your Unity computer vision implementation)._
4. Depth-Aware Immersive Visualization of Boundaries Using Particles in VR, 2025 _(Use to justify using Unity HDRP for spatial visualization)._