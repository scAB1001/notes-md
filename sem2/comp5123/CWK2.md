...
Whilst authenticating via SSH into the Cloud VM from the personal machine, which has a dynamically changing IP, Azure's automated security policies were triggered, generating a MicrosoftDefenderForCloud-JITRule (Just-In-Time access) within the Network Security Group (NSG). 
