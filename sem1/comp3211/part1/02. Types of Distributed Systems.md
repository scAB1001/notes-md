...
**(TRANSACTION PROCESSING MONITORS - TP MONITORS)** When transaction data is distributed across servers, a **TP Monitor** is the middleware component that coordinates the execution. It ensures the ACID properties are maintained using protocols (like two-phase commit) across all participating servers.  
![[tpm-in-a-ds.png|400]]
...
- **Remote Procedure Call (RPC) / Remote Method Invocation (RMI):** Allow a component to call a procedure/method on a remote component as if it were local. Leads to **tight coupling** (both parties must be running and know each other's exact location).
...
- **Core Requirements:**
    ...
    4. **Autonomy:** Self-managed with minimal human intervention (e.g., automatic IP address allocation via DHCP/Zeroconf, device discovery via UPnP, automatic software updates).
...
A subset of pervasive systems where devices change location and network attachment.
