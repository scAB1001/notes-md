...
#### d) Distributed Hash Tables (CORE system)
...
![[dht-ex.png]]
#### e) Hierarchical Approaches
Basic idea: build a large-scale search tree for which the
underlying network is divided into hierarchical domains. Each
domain is represented by a separate directory node
...
• Intermediate nodes contain a pointer to a child if and only if
the subtree rooted at the child stores an address of the entity
• The root knows about all entities.
Example: Storing information of entity E having two addresses in
different leaf domains (E is replicated in D1 and D2)
Lookup Operation
• Start lookup at local leaf node (Domain D)
• Node knows about E -> follow downward pointer,
else go up
• Upward lookup always stops at root
![[lookup-operation.png]]
Insert Operation
Entity E has created a replica in leaf domain D and needs to insert its address
